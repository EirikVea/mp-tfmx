...
player_NotePort(U32 note)
{
    ...
    if (uni.b.b0 < 0xC0) /* means normal note with finetune/macro ? */
    {
        ...
        c->ReallyWait = 1;
        c->NewStyleMacro = 0xFF;
        c->MacroPtr = macros[c->MacroNum=uni.b.b1];
    ...
    else if (uni.b.b0 < 0xF0) /* means note with portamento ? */
    {
        ...
        c->DestPeriod = (notevals[c->CurrNote = (uni.b.b0 & 0x3F)]);
    }
    else switch (uni.b.b0)
	{
	    case 0xF7: /* enve */
            c->EnvRate = uni.b.b1;
            ...
            break;
	    case 0xF6: /* vibr */
            c->VibTime=(c->VibReset=(uni.b.b1&0xFE))>>1;;
            c->VibWidth = uni.b.b3;
            c->VibFlag = 1; /* ?! */
            c->VibOffset = 0;
            break;
...
player_LoopOn(struct Hdb *hw)
{
	...
	hw->loop=(int(*)(struct Hdb*))&player_LoopOff;
	hw->c->MacroRun=0xFF;
	return 1;
}

static void
player_RunMacro(struct Cdb *c, U32 nChannel)
{
    UNI x;
    register int a=0;
    c->MacroWait=0;
	...
	case 0x13: /* dmaoff */
	    DEBUG(3) puts("DMAOff");
	    c->hw->loop=(int(*)(struct Hdb*))&player_LoopOff;
...
/*		if (c->SIDSize)
	...
	case 0x1A:
	    ...
	    c->hw->loop=(int(*)(struct Hdb*))&player_LoopOn;
	    c->hw->c=c;
...
/*		return;*/
	...
	case 0xD: /* add volume */
	    ...
	    if (x.b.b2!=0xFE)
	    {
			...
			S8 tempVol=(c->Velocity*3)+x.b.b3;
			if (tempVol>0x40)
				c->CurVol=0x40;
			else
				c->CurVol=tempVol;
...
void TFMXRewind()
{
	...
	hdb[0].vol = 0x40;
	hdb[0].mode = 3;
	hdb[0].loop = (int(*)(struct Hdb*))&player_LoopOff;
	hdb[0].loopcnt = 0;
	hdb[0].c = NULL;

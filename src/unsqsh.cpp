...
static int bfextu(ubyte *p,int bo,int bc);
static int bfexts(ubyte *p,int bo,int bc);

/* Recognize and return uncrunched length of the given buffer
 * Return 0 if not squashed.
 */

udword tfmx_sqsh_get_ulen (ubyte *buf, udword buflen)
{
  udword dstlen = 0; /* decrunched length of buffer */
